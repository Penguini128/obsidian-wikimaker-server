...
const removePublishedFile = (recordEntry) => {
...
app.post('/test-connection', (req, res) => {
    if (verifySecret(req.headers['authorization'])) res.status(200).send()
...
app.post('/remove-published-file', (req, res) => {
    ...
    removePublishedFile(recordEntry)
...
// Syncs the contents, location, and information about a server file
app.post('/sync-image-file', (req, res) => {
    // Verify secret
    const reqContent = req.body
    if (!verifySecret(req.headers['authorization'])) {
        res.status(202).send('Unauthorized')
